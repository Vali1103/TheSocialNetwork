...
# vali3
    def get_all_images(username):
        friends = get_current_friends(username)
        friends.append(username)  # Include și postările proprii
        select_query = """
            SELECT id, username, image, description, number_likes, number_comments, number_shares, comment_list, upload_date
            FROM images
            WHERE username IN ({})
            ORDER BY upload_date ASC
        """.format(','.join('?' for _ in friends))
        
        db_temp = get_db_imgs()
        cursor = db_temp.cursor()
        cursor.execute(select_query, friends)
        result = cursor.fetchall()
        images = []
        for row in result:
            images.append({
    ...
    def get_all_images_route():
        ...
        images = get_all_images(tokenVerify)
        return jsonify(images)

    def get_all_images_desc(username):
        friends = get_current_friends(username)
        friends.append(username)  # Include și postările proprii
        select_query = """
            SELECT id, username, image, description, number_likes, number_comments, number_shares, comment_list, upload_date
    ...
    def get_all_images_desc_route():
        ...
        images = get_all_images_desc(tokenVerify)
        return jsonify(images)

    def get_all_images_by_likes(username):
        friends = get_current_friends(username)
        friends.append(username)  # Include și postările proprii
        select_query = """
            SELECT id, username, image, description, number_likes, number_comments, number_shares, comment_list, upload_date
    ...
    def get_all_images_by_likes_route():
        ...
        images = get_all_images_by_likes(tokenVerify)
        return jsonify(images)
    ...
    def get_friends_request():
        ...
        if len(result) == 1 and result[0] is '':
